...
def build_workflow_plan(output_dir: Path, *, board_id: str | None = None) -> str:
    ...
    return "\n".join(
        [
            ...
            "",
            "```powershell",
            f"python scripts\\miro_rest_probe_board.py --output {_path_for_markdown(rest_manifest)}",
            "```",
            "",
            "## 2. Create or update the REST probe board",
            "",
            ...
            "",
            "```powershell",
            f"python scripts\\miro_rest_probe_board.py --execute{board_arg} --output {_path_for_markdown(rest_result)}",
            "```",
            "",
            "Local OAuth run:",
            "",
            "```powershell",
            f"python scripts\\miro_rest_probe_board.py --execute --oauth{board_arg} --output {_path_for_markdown(rest_result)}",
            "```",
            "",
